...
module.exports = {
  ...
  eleventyComputed: {
    ...
    description: (data) => `Get inspired by ${data.author}'s ${data.yearCount} ${data.subjects.length > 1 ? "multilingual " : ""}${data.keyboard} keymap and browse other ${oxfordJoin(data.documentFormat)} keymaps like this.`,
    ogImage: (data) => data.databaseImage,
    imageAlt: (data) => `${data.endbandPresent ? "Endband Present" : "Endband Absent"} ${data.textblockBindingRelationship}-relationship ${data.keyboard} with ${oxfordJoin(data.bindingType)} type.`
  }
};